...
export default class MyDatePicker extends Component {
    ...
    isReminderDay =day=> {
        for(const [index,value] of this.state.reminderDays.entries()) {
            if((value.date==day.date) && (value.month==day.month)){
                return true;
            }
        }
        return false;
        ...
        let days = this.state.monthDetails.map((day, index)=> {
            return (
                <div className={'c-day-container ' + (day.month !== 0 ? ' disabled' : '') + 
                    (this.isCurrentDay(day) ? ' highlight' : '') + (this.isSelectedDay(day) ? ' highlight-green' : '') + (this.isReminderDay(day) ? ' reminder-highlight' : '')} key={index}>
